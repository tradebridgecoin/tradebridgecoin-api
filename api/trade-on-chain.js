...
class Trade {
    ...
    //TODO to calculate real-time position market value stipulate historical data for category 3
    ...
    withdraw(tokensToWithdraw){
        ...
        this.cod.methods.withdraw(this.account, tokensToWithdraw * 1e6)
            ...
            .on('error', (err, receipt) => {
                if(receipt){
                    if(promiOrder.errFunc !== undefined)
                        promiOrder.errFunc(new Error(`out of gas: ${receipt.gasPrice}\t${receipt.gasUsed}`))
                } else{
                    if(promiOrder.errFunc !== undefined)
                        promiOrder.errFunc(err)
                }
            });
    ...
    transfer_token(beneficiary, tokens){
        ...
        if (tokens <= 0){
            if(promiOrder.errFunc !== undefined)
                promiOrder.errFunc(new Error('tokens to tranfers CANNOT be 0 or less'))
        } else{
            let w = this.wrapper;
            // let w3 = this.web3;
            ...
            this.token_balance()
                .then(balance => {
                    ...
                    if(balance < tokens){
                        ...
                        // reject.catch(e =>{});
                        if(promiOrder.errFunc !== undefined)
                            promiOrder.errFunc(new Error('tokens to tranfers CANNOT be greater than the balance'))
                    }else {
                        Object.prototype.wrapped = function() {
                            return w.wrap(this);
                        };
                        this.cod.methods.transfer(beneficiary.toLowerCase(), tokenDec6)
                            ...
                            .on('error',err => {
                                ...
                                // reject.catch(e =>{});
                                if(promiOrder.errFunc !== undefined)
                                    promiOrder.errFunc(new Error(err.message))
                            })
